...
<div class="container">
    ...
    <button class="btn btn-primary mt-4" onclick="window.location.href='/student/create'">Them</button>
...
<%--                <td>${student.id}</td>--%>
                ...
                <td>
                    <c:choose>
                        <c:when test="${student.score>=8}">Hoc sinh gioi</c:when>
                        <c:when test="${student.score>=6.5}">Hoc sinh kha</c:when>
                        <c:when test="${student.score>=5}">Hoc sinh trung binh</c:when>
                        <c:when test="${student.score>=3.5}">Hoc sinh yeu</c:when>
                        <c:otherwise>Cook</c:otherwise>
                    </c:choose>
                </td>
                <td>
                    <button class="btn btn-warning" onclick="window.location.href ='/student/update/${student.id}'">Sua</button>
                    <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal${student.id}">Xoa</button>
